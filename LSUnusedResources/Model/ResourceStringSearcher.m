...
- (BOOL)containsSimilarResourceName:(NSString *)resName {
    NSString *name = resName;
    NSString *nightStr = @"_night";
    NSRange nightRang = [resName rangeOfString:nightStr];
    if (nightRang.location != NSNotFound) {
        name = [resName substringToIndex:nightRang.location];
    }
...
- (NSArray *)createDefaultResourcePatternsWithResourceSuffixs:(NSArray *)resSuffixs {
    ...
    NSString *ojbcPattern = @"@\"(.*?)\""; // @"imageNamed:@\"(.+)\"";//or: (imageNamed|contentOfFile):@\"(.*)\"
